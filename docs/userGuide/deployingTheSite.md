...
1. Set the [`baseUrl` property of the `site.json` file](userGuide/siteConfiguration.html#baseUrl) to match the deploy location.
1. (Optional) Use the [`markbind serve` command](userGuide/cliCommands.html#serve-command) to stage the site locally and confirm the contents are as expected.
1. Use the [`markbind build` command](userGuide/cliCommands.html#build-command) to generate the site from source files. That command puts the generated site files in a directory named `_site` (you can change the output directory using parameters supplied to the command).
...
<panel type="seamless" header="**User Guide: Configuring the Site → `deploy`**" popup-url="userGuide/siteConfiguration.html#deploy">
